...
sites_by_windows = sites_by_windows %>% 
  arrange(start) %>%
  group_by(label)
sites_by_windows = subset(sites_by_windows, filter=="PASS")
...
cur_rnd_plot = genRFPlot(sites_by_windows, "roll.rnd.jc", "chr17", "Inversion 1", "log(RND)", ymax="logy", type="line", groups="label", features=inversions)
...
cur_rnd_plot = genRFPlot(sites_by_windows, "roll.rnd.jc", "chr17", "Inversion 2", "log(RND)", ymax="logy", type="line", groups="label", features=inversions, cur_feature=inversions[2,], num_flank_windows=50)
...
cur_rnd_plot = genRFPlot(sites_by_windows, "roll.rnd.jc", "chr17", "Inversion 3", "log(RND)", ymax="logy", type="line", groups="label", features=inversions, cur_feature=inversions[3,], num_flank_windows=50)
...
cur_rnd_plot = genRFPlot(sites_by_windows, "roll.rnd.jc", "chr17", "Inversion 4", "log(RND)", ymax="logy",, type="line", groups="label", features=inversions, cur_feature=inversions[4,], num_flank_windows=50)
...
doStats <- function(df, yvar, ylabel, filter_flag = FALSE){
  ...
  anova_qqplot = ggqqplot(df_anova_res) +
    ...
    annotate("text", x=2, y=2, label=paste("Shapiro p-value =", round(df_anova_shapiro$p.value, digits = 3)))
  print(anova_qqplot)
  # Plot a qqplot to visualize
  
  cat(paste("```Shapiro p-value (null: ANOVA residuals are normally distributed): ", round(df_anova_shapiro$p.value, digits = 3), "```\n"))
  ...
  cat(paste("```Kruskal p-value (null: groups are the same): ", round(df_kw$p.value, digits = 3), "```\n"))
...
for(pair in pairs){
  ...
  max_y = cur_data %>%
    filter(d.pair.jc < 1) %>%
    summarise(max = max(d.pair.jc, na.rm = TRUE)) %>%
    pull(max)
  
  d_plot = cur_data %>%
    filter(d.pair.jc < 1) %>%
    ggplot(aes(x=inv.overlap, y=d.pair.jc)) +
  ...
  max_y = max(cur_data$rnd.jc.log, na.rm = TRUE)
  ...
  rnd_plot = cur_data %>%
    ...
    #geom_point() +
    geom_boxplot(outlier.shape=NA) +
    coord_cartesian(ylim = c(NA, max_y + 1.4)) +
    xlab("") +
    ylab("log(RND)") + 
    bartheme() + 
    theme(axis.text.x = element_text(angle=25, hjust=1),
  ...
  pairwise_d_p = doStats(cur_data, "d.pair.jc", "d", filter_flag=TRUE)
  pairwise_rnd_p = doStats(cur_data, "rnd.jc.log", "log(RND)")
  ...
  cur_data_summary = cur_data %>% summarize(mean.dxy.jc=mean(d.pair.jc, na.rm=T), 
                                            median.dxy.jc=median(d.pair.jc, na.rm=T), 
                                            mean.rnd.jc=mean(rnd.jc, na.rm=T), 
                                            median.rnd.jc=median(rnd.jc, na.rm=T))
  
  summary_data = rbind(summary_data, cur_data_summary)
  cat("<br/>")
  print(
    cur_data %>% group_by(inv.overlap) %>%
    summarize(mean.dxy.jc=mean(d.pair.jc, na.rm=T), 
              median.dxy.jc=median(d.pair.jc, na.rm=T), 
              mean.rnd.jc=mean(rnd.jc, na.rm=T), 
              median.rnd.jc=median(rnd.jc, na.rm=T)) %>% 
    kable() %>% 
...
##########
  ...
  # d_dunn_df$conf.low <- d_dunn_df$Z - qnorm(0.975) * sqrt(1/(length(cur_data$d.pair.jc)))
  # d_dunn_df$conf.high <- d_dunn_df$Z + qnorm(0.975) * sqrt(1/(length(cur_data$d.pair.jc)))
  # 
  # d_dunn_df$sig = "N.S."
