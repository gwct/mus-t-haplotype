...
```{r non-inv-comps, warning=FALSE, results='asis', out.width="50%", fig.height=8}
...
for(pair in pairs){
  ...
  max_y = cur_data %>%
    filter(d.out.jc < 1) %>%
    summarise(max = max(d.out.jc, na.rm = TRUE)) %>%
    pull(max)
  ...
  pairwise_d_p = doStats(cur_data, "d.out.jc", "d", filter_flag=TRUE)
