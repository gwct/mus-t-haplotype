...
for(pair in pairs){
  ...
  d_kw = kruskal.test(d.out.jc ~ inv.overlap, data=cur_data)
  
  library(FSA)
  
  d_dunn = dunnTest(d.out.jc ~ inv.overlap, data=cur_data, method="bonferroni")
  d_dunn_df = as.data.frame(d_dunn$res)
  
  # Assume you have a data frame df, with divergence as your dependent variable 
  # and species as your independent variable
  ...
  ggplot(d_dunn_df, aes(x=reorder(Comparison, Z), y=Z, color=sig)) +
    geom_point() +
  ...
  leveneTest(anova_data$d.out.jc, anova_data$inv.overlap, center=mean)
  #bartlett.test(d.out.jc ~ inv.overlap, data=anova_data)
