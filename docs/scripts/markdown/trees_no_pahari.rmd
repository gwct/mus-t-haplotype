...
```{r dists, out.width="85%", fig.align = "center", warning=FALSE}

dists = rbind(inv_bp_dists, rand_dists)

x_comps = list(c("Inversion breakpoints", "Random windows"))

p = dists %>% 
  filter(wrf <= 0.5) %>%
  ggplot(aes(x=label, y=wrf, color=label)) +
...
p = dists %>% 
  filter(wrf <= 0.5) %>%
  ggplot(aes(x=adj, y=wrf, color=label)) +
...
```{r inversions, out.width="85%", fig.align = "center", warning=FALSE}

library(GenomicRanges)

inversions_bed_file = here("data", "inversions.bed")
inversions_bed = read.table(inversions_bed_file)
names(inversions_bed) = c("chr", "start", "end", "strand", "id")

all_windows_f$inv.cat = "Non-inversion"




mw_range = makeGRangesFromDataFrame(all_windows_f)
# Create range for marker windows

gene_range = makeGRangesFromDataFrame(inversions_bed, keep.extra.columns=TRUE)
gene_overlaps = findOverlaps(mw_range, gene_range)
all_windows_f$genes = gene_overlaps
