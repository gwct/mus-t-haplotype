...
cur_inversion = inversions[1,]

chrdata = subset(all_windows, chr==chrome_str & start > cur_inversion$start - 100 * window_size*1000 & end < cur_inversion$end + 100 * window_size*1000)
total_windows = length(chrdata[,1])
chrdata_f = subset(all_windows_f, chr==chrome_str & start > cur_inversion$start - 100 * window_size*1000 & end < cur_inversion$end + 100 * window_size*1000)
used_windows = length(chrdata_f[,1])
num_topos = max(chrdata_f$topo.num.chrome)
...
fig_2c = ggplot(chrdata, aes(x=start, y=ystart, color=col.cat, fill=col.cat)) +
  geom_rect(aes(ymin=ystart, ymax=yend, xmin=start,xmax=end)) +
  #geom_segment(aes(x=start, y=ystart, xend=start, yend=yend)) +
  scale_color_manual(values=cols_labs) +
  scale_fill_manual(values=cols_labs) +
  scale_y_continuous(limits=c(0,4), breaks=0.5:3.5, labels=c("Other", "3", "2", "1")) +
  #scale_x_continuous(limits=c(cur_inversion$start-500*window_size,cur_inversion$end+500*window_size), breaks=chr_breaks_5mb, labels=chr_labels_5mb, expand=c(0,0)) +
  geom_vline(xintercept=cur_inversion$start,color="black") +
  geom_vline(xintercept=cur_inversion$end,color="black") +
  ylab("Rank") +
  xlab("Position (Mb)") +
  ggtitle("Chromosome 17") +
  theme_classic() +
